...
## File that links genes and conigs
links  <- args[4]
...
write.table(t(c("scan","reachabilityDist")),paste(outdir, "reachabilityDistanceEstimates.tsv", sep="/"),row.names=F,col.names=F,quote=F,sep="\t")
write.table(t(c("first",est)),paste(outdir, "reachabilityDistanceEstimates.tsv", sep="/"),row.names=F,col.names=F,quote=F,sep="\t",append=T)
...
write.table(t(c("clusterName","cutoff")),paste(outdir, "bimodalClusterCutoffs.tsv", sep="/"),sep="\t",row.names=F,col.names=F,quote=F)
...
write.table(t(c("cluster","cutoff")),paste(outdir, "bimodalClusterCutoffs.tsv", sep="/"),sep="\t",col.names=F,row.names=F,quote=F)
for(ds in unique(clusterRes$cluster[grep("D",clusterRes$cluster)])){
	clusterRes <- muClus(ds,clusterRes,paste(outdir, "bimodalClusterCutoffs.tsv", sep="/"))
}

save.image(paste(outdir, "binny.Rdat", sep="/"))
...
for(bb in unique(clusterRes$cluster[grep("B",clusterRes$cluster)])){
  ...
  write.table(t(c(bb,estBB)),paste(outdir, "reachabilityDistanceEstimates.tsv", sep="/"),row.names=F,col.names=F,quote=F,sep="\t",append=T)
...
BBcdbTab <- data.frame("cluster"=names(table(BBcdb$cluster)),"contigs"=0,"numberEss"=0,"uniqueEss"=0,stringsAsFactors=F)
    ...
    write.table(BBcdbTab, paste(outdir, paste("blob1Cluster",bb,".tsv",sep=""), sep="/"),sep="\t",row.names=F,quote=F)
    ...
    clusterRes$cluster[clusterRes$contig %in% BBduCont] <- paste(gsub("B","D",bb),
                                                               ...
                                                               sep=".")
    for(bds in uniD){
      clusterRes <- muClus(bds,clusterRes, paste(outdir, "bimodalClusterCutoffs.tsv", sep = "/"))
    }
  }
}
...
BBcdbTab <- data.frame("cluster"=names(table(BBcdb$cluster)),"contigs"=0,"numberEss"=0,"uniqueEss"=0,stringsAsFactors=F)
    ...
    write.table(BBcdbTab,paste(outdir, paste("blob2Cluster",bb,".tsv",sep=""), sep = "/"),sep="\t",row.names=F,quote=F)
...
BBcdbTab <- data.frame("cluster"=names(table(BBcdb$cluster)),"contigs"=0,"numberEss"=0,"uniqueEss"=0,stringsAsFactors=F)
    ...
    write.table(BBcdbTab,paste(outdir, paste("blob3Cluster",bb,".tsv",sep=""), sep = "/" ), sep="\t",row.names=F,quote=F)
...
write.table(clusterRes,paste(outdir, "contigs2clusters.tsv", sep = "/"), sep="\t",row.names=F,quote=F)
saveRDS(clusterRes,paste(outdir, "contigs2clusters.RDS", sep = "/"))

print("Saved tables. Plotting final map now.")

essPal <- colorRampPalette(brewer.pal(11,"Spectral"))(111)[111:1]
pdf(paste(outdir, "finalClusterMap.pdf" , sep = "/"),width=6,height=5,pointsize=8)
...
for(i in 1:nrow(cI2)){
  ...
  if(cI2$essNum[i]>15&clus!="N") text(cI2$medX[i],cI2$high[i]+0.5,labels=clus,cex=0.8,font=2,col=pc,pos=3)
  rm(pc)
}
mtext(sample,3,0)
...
#lines(c(0,hi$counts[1]*2.4/max(hi$counts[1])),c(1,1),col="white")
