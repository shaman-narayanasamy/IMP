...
$('#annottab').on('show.bs.tab', function (e) {
    if($("#ifr-kronaplot").attr('src') === undefined){
        $("#ifr-kronaplot").attr('src', 'Analysis/results/mt.gene_kegg_krona.html');
    }
});

// load maxbin results
$.get('Binning/MaxBin/maxbin_res.log', function(data) {
   $('#maxbin-results').append(data);
}, 'text');
